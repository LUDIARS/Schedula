...
## ⚠️ このリポジトリは Actio から分離復活した予定基盤 (2026-05-20)

Schedula は Actio を `git clone` して復活させた **予定 (Event) / カレンダー
管理基盤**。 Actio が「予定 + タスク」 を抱えて肥大化したため、 予定軸を
Schedula に再分離している。

- **Schedula が持つ**: event / calendar / myplan / smart-scheduler / holiday /
  schedule / school (facility-booking 除く) / voting / placement + 共通基盤
- **Actio に残す (Schedula から除去)**: `modules/task/` / `modules/pm/` /
  `src/task-plugins.ts`
- **Aedilis へ移す (Schedula から除去)**: `modules/school/facility-booking/`、
  `modules/reservation/` は破棄
- 現在 `feat/split-from-actio` ブランチでタスク系の除去作業中。 移行計画は
  `DESIGN.md` §6

> 注意: 下記の「アーキテクチャ」 「既存リポジトリ」 等の節は分離前 Actio の
> 全体像を含む。 タスク系モジュール除去に伴って順次更新する。 タスク (Task)
> 関連の記述は除去対象。
...
全チェックは `scripts/ci-check.sh` に一元管理されています。GitHub Schedulans (`.github/workflows/test.yml`) とローカル pre-push hook (`.claude/settings.json`) の両方がこのスクリプトを呼び出します。
